...
-- course_code/tags there. Normalize both into indexed columns so the match
-- happens as a real SQL WHERE clause instead.
--
-- Originally written as generated STORED columns, but Postgres rejects that
-- with "generation expression is not immutable" for lower()/regexp_replace()
-- on this instance's collation — a trigger keeps the same effect (kept in
-- sync on every insert/update) without that restriction.
...
create or replace function listings_set_normalized_search_columns()
returns trigger language plpgsql as $$
begin
  new.course_code_normalized := regexp_replace(lower(coalesce(new.course_code, '')), '[^a-z0-9]', '', 'g');
  new.tags_normalized := regexp_replace(lower(coalesce(array_to_string(new.tags, ' '), '')), '[^a-z0-9 ]', '', 'g');
  return new;
end;
$$;

drop trigger if exists listings_normalized_search_columns_trigger on listings;
create trigger listings_normalized_search_columns_trigger
  before insert or update on listings
  for each row execute function listings_set_normalized_search_columns();

-- Backfill existing rows (the trigger only covers future inserts/updates).
update listings set
  course_code_normalized = regexp_replace(lower(coalesce(course_code, '')), '[^a-z0-9]', '', 'g'),
  tags_normalized = regexp_replace(lower(coalesce(array_to_string(tags, ' '), '')), '[^a-z0-9 ]', '', 'g');
