...
Run every file in `supabase/migrations/` in filename order (timestamp-prefixed, so alphabetical = chronological). This includes RLS/function-security hardening — without it, anyone with the anon key (which ships in the browser bundle) can read/write every table directly via the REST API. Do not skip any file in the sequence.
